...
It writes log entries to ``/var/log/syslog`` as well as a Dokku-specific logs sub-directory ``/var/log/dokku/events.log``, which can be easily displayed:
...
List all events that are currently being recored:
```
& dokku events:list
