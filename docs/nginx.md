...
[See the full documentation](http://progrium.viewdocs.io/dokku/deployment/ssl-configuration).
...
The domains plugin allows you to specify custom domains for applications. This plugin is aware of any ssl certificates that are imported via the `certs` plugin. Be aware that setting `NO_VHOST` will override any custom domains.
