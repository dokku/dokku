...
assert_nginx_property_set_get() {
  ...
  run /bin/bash -c "dokku --quiet nginx:report $TEST_APP --format json | jq -r '.\"nginx-global-$prop\"'"
  assert_success
  assert_output "$global_value"

  run /bin/bash -c "dokku --quiet nginx:report $TEST_APP --format json | jq -r '.\"nginx-computed-$prop\"'"
  assert_success
  assert_output "$global_value"
  ...
  run /bin/bash -c "dokku --quiet nginx:report $TEST_APP --format json | jq -r '.\"nginx-$prop\"'"
  assert_success
