...
setup() {
  ...
  dokku traefik:start
  create_app
  dokku proxy:set "$TEST_APP" traefik
}
...
@test "(proxy-route:traefik) basic routing - /api/v0 hits api, / hits web" {
...
@test "(proxy-route:traefik) removing a route falls back to web" {
  run deploy_app python "dokku@$DOKKU_DOMAIN:$TEST_APP" add_api_process_callback
  assert_success
