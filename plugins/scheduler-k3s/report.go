...
func ReportSingleApp(appName string, format string, infoFlag string) error {
	...
	if appName == "--global" {
		...
		flags = map[string]common.ReportFunc{
			...
			"--scheduler-k3s-computed-node-sysctls-image":       reportComputedNodeSysctlsImage,
			"--scheduler-k3s-global-node-sysctls-image":         reportGlobalNodeSysctlsImage,
			"--scheduler-k3s-computed-node-sysctls-pause-image": reportComputedNodeSysctlsPauseImage,
...
func reportComputedNodeSysctlsImage(appName string) string {
	return getComputedNodeSysctlsImage()
}

func reportGlobalNodeSysctlsImage(appName string) string {
	return getGlobalNodeSysctlsImage()
}

func reportComputedNodeSysctlsPauseImage(appName string) string {
	return getComputedNodeSysctlsPauseImage()
}

func reportGlobalNodeSysctlsPauseImage(appName string) string {
	return getGlobalNodeSysctlsPauseImage()
}
