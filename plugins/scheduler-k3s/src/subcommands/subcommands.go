...
func main() {
	...
	case "preview":
		...
		args.Parse(os.Args[2:])
		appName := args.Arg(0)
		err = scheduler_k3s.CommandPreview(appName, *showSecrets, *showSecretsDecoded)
	case "profiles:add":
	...
	case "report":
		...
		format := args.String("format", "stdout", "format: [ stdout | json ]")
		reportArgs, flagErr := common.ParseReportArgs("scheduler-k3s", os.Args[2:])
		if flagErr == nil {
			args.Parse(reportArgs.OSArgs)
			appName := args.Arg(0)
			...
			err = scheduler_k3s.CommandReport(appName, *format, reportArgs.InfoFlag)
		}
	case "set":
